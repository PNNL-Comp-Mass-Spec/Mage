...
namespace MageExtExtractionFilters
{
    ...
    internal class MergeProteinData
    {
        ...
        private Dictionary<string, int> mProteinDataLookup;
        ...
        public MergeProteinData(MergeModeConstants mergeMode)
        {
            MergeMode = mergeMode;
        }
        ...
        /// </summary>
        /// <param name="outRow">Output row</param>
        ...
        public bool MergeFirstProtein(string[] outRow, out string warningMessage)
        {
            ...
            if (!mProteinDataLookup.TryGetValue(resultID, out var sequenceID))
            {
        ...
        public Collection<string[]> MergeAllProteins(string[] outRow, out bool matchFound)
        {
            ...
            // If single protein, outRow has been updated, and no further action taken.
        ...
        public void GetProteinLookupData(ResultType.MergeFile mapMergeFile, ResultType.MergeFile proteinMergeFile, string resultDirectoryPath)
        {
            ...
            var proteinFileName = proteinMergeFile.MergeFileName;

            if (!string.IsNullOrEmpty(proteinFileName))
            {
                var proteinFileReader = new DelimitedFileReader
                {
                    FilePath = Path.Combine(resultDirectoryPath, proteinFileName)
                };

                ProcessingPipeline.Assemble("Lookup Protein Data", proteinFileReader, mProteinData).RunRoot(null);
            }
