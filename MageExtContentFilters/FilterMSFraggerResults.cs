...
namespace MageExtContentFilters
{
    public class FilterMSFraggerResults : FilterResultsBase
    {
        ...
        public FilterMSFraggerResults(IEnumerable<string[]> filterCriteria, string filterSetID)
            : base(filterCriteria, filterSetID)
        {
        }

        public bool EvaluateMSFragger(
            ...
            foreach (var filterGroupID in m_FilterGroups.Keys)
            {
                ...
                foreach (var filterRow in m_FilterGroups[filterGroupID])
                {
                    ...
                    switch (currentCriteriaName)
                    {
                        ...
                        case "MSGF_SpecProb":
                            // We don't run MSGF on MSFragger results, but we will test the filter threshold against specEValue
